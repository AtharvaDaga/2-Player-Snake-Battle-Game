...
while winner == None:
    ...
    ## move head for player 1 (AI controlled)
    if food_drawn:
        dx = food_x - p1.x
        dy = food_y - p1.y
        if dx != 0:
            p1.direction = (1 if dx > 0 else -1, 0)
        elif dy != 0:
            p1.direction = (0, 1 if dy > 0 else -1)
        else:
            p1.direction = random.choice([(1, 0), (-1, 0), (0, 1), (0, -1)])  # Random move if on food
        p1.x += p1.direction[0]
        p1.y += p1.direction[1]
    else:
        p1.direction = random.choice([(1, 0), (-1, 0), (0, 1), (0, -1)])  # Random move if no food
        p1.x += p1.direction[0]
        p1.y += p1.direction[1]

    ## move head for player 2
    p2.x += p2.direction[0]
    p2.y += p2.direction[1]
...
if winner == 0:
    ...
    print("Player 1 wins with a score of:", p1.length)
elif winner == 2:
    if args.raspi:
        print("AI Player wins with a score of:", p2.length)
    else:
        print("Player 2 wins with a score of:", p2.length)
